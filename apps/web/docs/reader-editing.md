...
Entering Edit preserves the reading position: the editor is populated before paint and focused without scrolling, the title retains its layout, and editing instructions stay in the fixed toolbar.
...
- Scroll regression check with a 60-paragraph local article: Edit preserves both scroll offset and content position halfway down (5,760px) and at the bottom (12,311.5px). Typing in the visible paragraph, Undo, Done, and Cancel preserve the position too.
